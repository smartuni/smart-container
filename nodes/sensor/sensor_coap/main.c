...
// declare functions
static size_t send_req(char *addr, char *port, char *path, char *data);
static size_t get_sensor_value(void);
...
int main(void)
{
    ...
    sensorReading = get_sensor_value();

    printf("Send msg with payload %c\n", sensorReading);

    if (!send_req("ff02::1", "5683", "/waterleak", "1")) {
        puts("msg send");
    } else
    puts("msg send failed");
...
static size_t send_req(char *addr, char *port, char *path, char *data)
{
    ...
    if(data != NULL) {
        ...
        gcoap_req_init(&pdu, buf, CONFIG_GCOAP_PDU_BUF_SIZE, COAP_GET, path);
